...
export class BatchProcessor {
    ...
    async analyzeNodes(nodes, onProgress) {
    ...
    async processNodes(analysisResults, onProgress) {
        this.isProcessing = true;
        this.isCancelled = false;
        const results = [];
        ...
        try {
            ...
            const { processable, issues } = await this.fontManager.validateNodesForProcessing(nodesToProcess.map(r => r.node));
            ...
            for (let i = 0; i < processable.length; i += CHUNK_SIZE) {
                ...
                for (let j = 0; j < chunk.length; j++) {
                    ...
                    try {
                        ...
                        // Generate changes
                        const changes = this.processor.generateChanges(analysisResult.originalText, analysisResult.issues);
                        // Apply changes
    ...
    async processSelectedNodes(onProgress) {
    ...
    async scanCurrentPage(onProgress) {
    ...
    async processIndividualNode(node, forceChanges) {
        var _a;
        try {
            // Validate node
            ...
            if (processable.length === 0) {
                return {
                    node,
                    success: false,
                    error: ((_a = issues[0]) === null || _a === void 0 ? void 0 : _a.reason) || 'Node cannot be processed'
                };
            }
